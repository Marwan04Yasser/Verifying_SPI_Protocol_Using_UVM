package SPI_wrapper_write_read_seq_pkg;
  ...
  class SPI_wrapper_write_read_sequence extends uvm_sequence #(SPI_wrapper_seq_item);
    ...
    task body();
      ...
      repeat (10000) begin
        start_item(seq_itm);
        seq_itm.mode_select = SPI_wrapper_seq_item::MODE_MIXED;
        seq_itm.write_mode_const.constraint_mode(0);
        seq_itm.read_mode_const.constraint_mode(0);
        seq_itm.mixed_mode_const.constraint_mode(1);
        ...
        if (!seq_itm.randomize() with {
           ...
           }) `uvm_error("RANDOMIZE_FAIL","write_read_sequence randomize failed")
        finish_item(seq_itm);
      end
    endtask
  endclass
